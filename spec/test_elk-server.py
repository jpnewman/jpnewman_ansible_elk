...
def test_redis_transparent_hugepage_enabled(Command):
    command = Command('sudo cat /sys/kernel/mm/transparent_hugepage/enabled')
    assert '[never]' in command.stdout.strip()


def test_redis_transparent_hugepage_defrag(Command):
    command = Command('sudo cat /sys/kernel/mm/transparent_hugepage/defrag')
    assert '[never]' in command.stdout.strip()
...
def test_nginx_is_configtest(Command):
    command = Command('sudo service nginx configtest')
